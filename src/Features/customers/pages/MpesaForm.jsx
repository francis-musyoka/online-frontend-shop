...
import { axiosInstance, PATCH_ROUTES, POST_ROUTES } from '../../../constant';
...
const MpesaForm = (props) => {
    ...
    const handleSave = async() =>{
        if(validatePhoneNumber){
            try {
                const {data} = await axiosInstance.post(POST_ROUTES.MPESA_PAYMENT,{phoneNumber,countryCode});
    ...
    const handleSaveChanges = async()=>{
        if(validatePhoneNumber){
            try {
                const {data} = await axiosInstance.patch(PATCH_ROUTES.UPDATE_MPESA_PAYMENT(paymentDetails.id),{phoneNumber});
    ...
    const handleDelete = async()=>{
        try {
            await axiosInstance.post(POST_ROUTES.DELETE_MPESA_PAYMENT(paymentDetails.id));
            setDisplayForm(true);
