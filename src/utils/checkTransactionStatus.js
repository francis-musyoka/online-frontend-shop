import { GET_ROUTES, PATCH_ROUTES, PATH_URL, POST_ROUTES } from "../constant";
...
export const pollTransactionStatus = async (transactionId, showToast, setPollStatus, orderNumber, navigate) => {
    ...
    const checkStatus = async (attempts = 0) => {
        ...
        try {
            const { data } = await axiosCustomer.get(GET_ROUTES.GET_MPESA_TRANSACTION_STATUS(transactionId));
            if (data.success) {
                if (data.transactionDetails.status === 'completed') {
                    ...
                    try {
                        const response = await axiosCustomer.patch(PATCH_ROUTES.UPDATE_ORDER(orderNumber));
                    ...
                    try {
                        const clearCartResponst = await axiosCustomer.post(POST_ROUTES.CLEAR_CART);
